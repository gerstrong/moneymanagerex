...
{
    FieldData *field_n = FieldModel::instance().unsafe_get_data_n(m_field_id);
    if (!field_n)
        return;
...
{
    const FieldData *field_n = FieldModel::instance().get_data_n(m_field_id);
    if (!field_n)
        return;
...
{
    const FieldData *field = FieldModel::instance().get_data_n(m_field_id);
    if (!field)
        return;
