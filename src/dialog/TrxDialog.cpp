...
bool TrxDialog::ValidateData()
{
    ...
    if (!m_transfer) {
        ...
        m_journal_data.PAYEEID = payee_n->m_id;
        if (!TrxModel::is_foreign(m_journal_data)) {
            m_journal_data.TOACCOUNTID = -1;
        }
...
void TrxDialog::SetCategoryForPayee(const PayeeData *payee_n)
{
    ...
    if ((PrefModel::instance().getTransCategoryNone() == PrefModel::LASTUSED ||
        ...
        const CategoryData* category_n = CategoryModel::instance().get_data_n(payee_n->m_category_id);
        if (category_n) {
...
void TrxDialog::OnAutoTransNum(wxCommandEvent& WXUNUSED(event))
    ...
    const auto numbers = TrxModel::instance().find(
        TrxCol::ACCOUNTID(OP_EQ, m_journal_data.ACCOUNTID),
...
void TrxDialog::OnOk(wxCommandEvent& event)
{
    ...
    TrxData trx_d;
    if (m_mode == MODE_EDIT) {
        trx_n = TrxModel::instance().unsafe_get_data_n(m_journal_data.TRANSID);
    }
    else {
        trx_d = TrxData();
        trx_n = &trx_d;
